...
//Rare exceptions to indicate a bug in the _xox code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_xox = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_xox]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.xox.module"
